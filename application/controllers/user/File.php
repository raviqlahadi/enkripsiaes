...
class File extends MY_Controller{
  ...
  private $name_file = [
    ...
    public function index(){
      ...
      $file = $this->m_file->getWhere(array('owner_id' => $user_id));
      $file_from = $this->m_file->getWhere(array('user_id' => $user_id));
      $file_info = [];
      foreach($file as $k => $f){
        $file_info[$k] = $this->file_info($f);
      }

      foreach ($file_from as $k => $d) {
        array_push($file_info, $this->file_info($d));
